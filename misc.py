...
def zoom_to_size(x, out_size_x, out_size_y):  
    s = tf.shape(x)  
    factor_x = out_size_x / s[2]
    factor_y = out_size_y / s[3]
    x = tf.reshape(x, [-1, s[1], s[2], 1, s[3], 1])
    x = tf.tile(x, [1, 1, 1, factor_x, 1, factor_y])
    x = tf.reshape(x, [-1, s[1], out_size_x, out_size_y])
    return x
...
def create_image_grid(images, grid_size=None):
    ...
    whiteedge = 15  # whiteedge between outputed images

    grid = np.ones(list(images.shape[1:-2]) + [grid_h * (img_h+whiteedge) - whiteedge, grid_w * (img_w+whiteedge) - whiteedge], dtype=images.dtype)
    for idx in range(num):
        x = (idx % grid_w) * (img_w+whiteedge)
        y = (idx // grid_w) * (img_h+whiteedge)
