...
def shape_to_list(shape):
    return [dim for dim in shape]# [dim.value for dim in shape]  #
...
class Network:
    ...
    def __setstate__(self, state):
        ...
        # Depending on the cudnn versions, this following line may not need, otherwise error reported ('argument 0 of dimension....')
    ...
    def copy_vars_from(self, src_net):
       ...
       #     return value
       # set_vars({self.find_var(name): flip_value(value) for name, value in name_to_value.items()})
    ...
    def copy_trainables_from(self, src_net):
        ...
        set_vars({self.find_var(name): value for name, value in name_to_value.items()})
       # set_vars({self.find_var(name): np.flip(value, 0) for name, value in name_to_value.items()})
