...
def G_wgan_acgan(G, D, lod, 
                 ...
                 GANloss_std = 16.,     # global features' mean and std are not included here, can included them if needed.
                 wellloss_mean = 0.,
                 wellloss_std = 1.,
                 probloss_mean = 0.,
                 probloss_std = 1.,
    ...
    fake_imgs_out = G.get_output_for(latents, labels_lg_img, well_facies_lg, prob_imgs_lg, is_training=True) # shape of [N, 3, 128, 128]  
        ...
        with tf.name_scope('LabelPenalty'):
            if 'pb_cf_prop' in labeltypes:
                def add_pb_cf_penalty(weight):
                    ...
                    if lossnorm: pb_cfPenalty = (pb_cfPenalty -1.10) / 0.41  # To normalize this loss 
            ...
            if 'fp_prop' in labeltypes:
                def add_fpprop_penalty(weight):
                    ...
                    if lossnorm: fppropPenalty = (fppropPenalty -1.78) / 1.13  # To normalize this label_penalty 
...
def D_wgangp_acgan(G, D, lod, opt, minibatch_size, reals, labels, well_facies, prob_imgs, facies_codes,
    ...
    fake_imgs_out = G.get_output_for(latents, labels_img, well_facies, prob_imgs, is_training=True)  # shape of [N, 3, 128, 128]  
    reals_input = indicatorize(reals, facies_codes) if facies_indic else reals     # indicators of real facies imgs; shape of [N, 3, 128, 128, 32] 
